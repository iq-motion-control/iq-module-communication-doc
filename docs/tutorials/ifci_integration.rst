...
****************************************************
IFCI Integration with PX4 and ArduPilot
****************************************************
...
Now, in Vertiq IO, set VTQ_VELO_CUTOFF to the desired velocity that prevents pulsing below the specified velocity, for example, you can set it to 50. Also set the Control Value Indices (CVIs) for VTQ_THROTTLE_CIV to 0, VTQ_X_CVI to 1, and VTQ_Y_CVI to 2.
...
For this basic throttle only test, only 1 Control Value is needed for the throttle output to the one connected module. Since there is only 1 CV, it is placed in index 0. Set SERVO_VIQ_CVS which is explained in :ref:`building ArduPilot <ifci_px4_flight_controller>` to 1. We will also configure our flight controller to receive telemetry from our connected module. Since for this example, the module was reset to its factory default settings, the module currently has Module ID 0 <link: somewhere explains module ID>. The lowest bit in the bitmask represents Module ID 0, so set SERVO_VIQ_TEL_BM to 1 :ref:`building ArduPilot <ifci_px4_flight_controller>`. Write these parameters..
...
Now that we can perform basic control of, and telemetry reception from our two modules, we’ll configure the module and flight controller for pulsing.
