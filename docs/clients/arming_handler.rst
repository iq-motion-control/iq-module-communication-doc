...
functions, except for motor_armed.
...
creation of a ArmingHandler object. See Table 11 for available messages. All message objects
use the Short Name with a trailing underscore. All messages use the standard Get/Set/Save functions, except for motor_armed.
...
.. code-block:: c++
    ...
    void main(){
        ...
        ArmingHandlerClient temp_client(0);

        // Use the Temperature Estimator Client
        temp_client.always_armed_.get(com)
...
All message strings use the Short Names. All messages use the standard Get/Set/Save functions, except for motor_armed.
...
.. code-block:: Matlab
    ...
    tes = ArmingHandlerClient(’com’,com);
    % Use the ArmingHandlerClient object
    alwaysArmed = tes.get(’always_armed’);
...
All messages use the standard Get/Set/Save functions, except motor_armed.
...
+========+=====================================+================+===========+===========+========================================================================================================================================================================================================================================+
| 1      | always_armed                        | get, set, save | uint8     |           | If True, handler will be automatically armed.                                                                                                                                                                                          |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 2      | arm_on_throttle                     | get, set, save | uint8     |           | If True, throttle will be used to arm.                                                                                                                                                                                                 |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 3      | arm_throttle_upper_limit            | get, set, save | float     | %/100     | Upper limit for throttle to arm.                                                                                                                                                                                                       |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 4      | arm_throttle_lower_limit            | get, set, save | float     | %/100     | Lower limit for throttle to arm.                                                                                                                                                                                                       |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 5      | disarm_on_throttle                  | get, set, save | uint8     |           | If True, throttle will be used to disarm.                                                                                                                                                                                              |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 6      | disarm_throttle_upper_limit         | get, set, save | float     | %/100     | Upper limit for throttle to disarm.                                                                                                                                                                                                    |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 7      | disarm_throttle_lower_limit         | get, set, save | float     | %/100     | Lower limit for throttle to disarm.                                                                                                                                                                                                    |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 8      | consecutive_arming_throttles_to_arm | get, set, save | uint32    | throttles | Number of consecutive throttles before arming.                                                                                                                                                                                         |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 10     | disarm_song_option                  | get, set, save | uint8     |           | The state that determines if an dhow many times the module will play its disarm song after coming to a stop. See row below for each state:                                                                                             |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 11     | manual_arming_throttle_source       | get, set, save | uint8     |           | The throttle source used as its armed throttle source for manual arming. The default manual throttle source is Unknown. In order to use manual arming, a throttle source must be set first. See row below for each state:              |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
||       ||                                    ||               ||          ||          || 0 = Unknown:                                                                                                                                                                                                                          |
||       ||                                    ||               ||          ||          || 1 = Hobby: REFER TO Throttle Sources page when merged in docs                                                                                                                                                                         |
||       ||                                    ||               ||          ||          || 2 = DroneCAN:                                                                                                                                                                                                                         |
||       ||                                    ||               ||          ||          || 3 = IQUART:                                                                                                                                                                                                                           |
+--------+-------------------------------------+----------------+-----------+-----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
