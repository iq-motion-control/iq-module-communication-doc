...
.. note::
    This feature is only available on speed firmwares of version 0.2.0 or greater, earlier firmwares did not support this feature.

.. warning::
    This is an advanced feature, and it can result in reductions of the step response of the motor or stability issues if configured improperly.
    Refer to the full documentation below for details on how to properly configure this feature.

The closed loop supply current limiter is a feature that can be used to limit the supply current drawn or regenerated by the module when it is motoring or regenerating. The module restricts
...
The supply current limit when motoring can be configured using the ``motoring_supply_current_limit`` entry, and the supply current limit when regenerating can be configured
using the ``regen_supply_current_limit`` entry. From the module's perspective, the supply current when motoring is always positive, and the supply current when regenerating is
always negative. Therefore, the ``motoring_supply_current_limit`` should always be a positive number, and the ``regen_supply_current_limit`` should always be a negative number.
