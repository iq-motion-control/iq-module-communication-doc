...
.. note:: If your flight controller expects to receive ERPM data from these telemetry messages, it may not interpret the RPM data sent by Vertiq modules correctly by default.
    Without changing your module's settings, additional configuration may be necessary on the flight controller to properly interpret the received RPM data as RPM and not as ERPM. For detailed information on how PX4 
